...
/**
 * Obtener último número de comprobante autorizado (para cualquier tipo de comprobante)
 */
export async function getLastVoucherNumberByType(pointOfSale: number, voucherType: VoucherType): Promise<number> {
  const credentials = await getWSAACredentials('wsfe');
  const cuitEmisor = await getCuitEmisor();
  const wsfeUrl = await getWSFEUrl();
  const cbte_tipo = VOUCHER_TYPE_CODES[voucherType];
...
/**
 * Obtener último número de comprobante autorizado (compatibilidad con código existente)
 */
...
/**
 * Crear Nota de Crédito electrónica y obtener CAE
 * Según documentación AFIP wsfev1: https://www.afip.gob.ar/ws/documentacion/ws-factura-electronica.asp
 */
export async function createCreditNote(request: CreateCreditNoteRequest): Promise<InvoiceResponse> {
  const credentials = await getWSAACredentials('wsfe');
  const cuitEmisor = await getCuitEmisor();
  const wsfeUrl = await getWSFEUrl();
  const cbte_tipo = VOUCHER_TYPE_CODES[request.creditNoteType];
  
  // Obtener último número y calcular el siguiente
  const lastNumber = await getLastVoucherNumberByType(request.pointOfSale, request.creditNoteType);
  const nextNumber = lastNumber + 1;
  ...
  // Datos del comprobante asociado (obligatorio para NC)
  const assocVoucher = request.associatedVoucher;
  
  // Armar request SOAP para FECAESolicitar con CbtesAsoc
  const soapRequest = `<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ar="http://ar.gov.afip.dif.FEV1/">
  <soap:Body>
    <ar:FECAESolicitar>
      ...
      <ar:FeCAEReq>
        ...
        <ar:FeDetReq>
          <ar:FECAEDetRequest>
            ...
            <ar:CbtesAsoc>
              <ar:CbteAsoc>
                <ar:Tipo>${assocVoucher.type}</ar:Tipo>
                <ar:PtoVta>${assocVoucher.pointOfSale}</ar:PtoVta>
                <ar:Nro>${assocVoucher.number}</ar:Nro>
                ${assocVoucher.cuit ? `<ar:Cuit>${assocVoucher.cuit.replace(/-/g, '')}</ar:Cuit>` : ''}
                ${assocVoucher.date ? `<ar:CbteFch>${assocVoucher.date}</ar:CbteFch>` : ''}
              </ar:CbteAsoc>
            </ar:CbtesAsoc>
            <ar:Iva>
              <ar:AlicIva>
  ...
  try {
    ...
    const xmlText = await response.text();
    console.log('WSFE NC Response:', xmlText);
